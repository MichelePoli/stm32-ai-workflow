...
Given the challenges of conducting a large-scale empirical developer study, the time estimates for the manual workflows were established using an analytical evaluation methodology inspired by the \textbf{Keystroke-Level Model (KLM)} and \textbf{GOMS} frameworks \cite{card1980keystroke, baek2014evaluating}. By decomposing the manual development lifecycle (navigating the STM32CubeMX GUI, configuring peripheral clock trees, and manually porting Keras artifacts into the X-CUBE-AI generator) into discrete operative steps (pointing, clicking, mental preparation, and system response times), we established a baseline for a proficient user. These estimates were then cross-referenced with three complementary sources to verify their plausibility:
...
For instance, recent studies on embedded ML CI/CD pipelines report a 70\% reduction in deployment time \cite{embedded_ml_cicd_2025}; the NNI experiment generation in this framework exceeds that benchmark, reducing a 60-minute task to under 2 minutes (97\% savings).
...
Finally, hidden costs for initial framework setup (configuring Docker containers and downloading STM32Cube packages) are excluded from these timings, though this overhead becomes negligible when amortized across multiple projects.
...
\begin{enumerate}
    ...
    \item \textbf{Scenario 3: Scalable Cluster with Triton Inference Server}\\
          To resolve the resource locking issue, the final architecture decoupled the multi-agent orchestration logic from the model inference engine. User-specific LangGraph containers now run exclusively on CPU and RAM, delegating all LLM requests to a centralized \textbf{NVIDIA Triton Inference Server} via standardized APIs \cite{nvidia_triton_docs}. Triton acts as an optimized proxy, dynamically loading and unloading models from GPU memory, handling concurrent request batching \cite{kwon2023efficientmemorymanagementlarge}, and enabling fractional GPU allocation. This decoupling allows multiple agentic pipelines to share the same hardware accelerators simultaneously, laying the groundwork for a fully scalable Kubernetes deployment.
\end{enumerate}

Decoupling workflow agents from model inference eliminates the 1:1 GPU-per-user binding, raising concurrent throughput and removing the memory saturation failures that appeared under the single-container architecture.
...
The quantitative analysis shows the automated pipeline cuts the Edge AI deployment lifecycle from roughly 4 hours to under 30 minutes, a 9× reduction that holds up against published MLOps benchmarks while covering the microcontroller-specific steps (firmware generation and resource validation) that general-purpose tools omit.
