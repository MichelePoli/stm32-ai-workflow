...
\begin{enumerate}
    ...
    \item \textbf{Scenario 2: Remote Research Server (edaserver1) and Containerization}\\
          This architecture migrated to a high-performance research server (\texttt{edaserver1@polito.it}) equipped with a dedicated NVIDIA A4000 GPU (16~GB VRAM). The migration happened in two distinct stages.
          
          \textbf{Phase A: Interactive Remote Development.} Before containerizing the application, developers accessed the server directly via SSH using VSCode Remote. They launched the orchestration logic natively using \texttt{langgraph dev}. This command started LangGraph Studio right on the server. Developers could visually debug the state machine, inspect memory payloads, and step through node executions without overloading their personal laptops. The 16~GB GPU effortlessly handled a single Ollama instance (Mistral) serving these interactive debugging sessions.

          \textbf{Phase B: Docker Implementation and Memory Bottleneck.} Once the core graph stabilized, the system moved to individual Docker containers per user. Each container embedded its own isolated instance of Ollama to handle LLM requests. This 1:1 mapping between user containers and GPU resources quickly exposed a severe infrastructure limit. The most critical failure occurred when two simultaneous users attempted to run different models, such as Mistral and \texttt{gpt-oss-20b}. The 16~GB GPU could not store both models at once. This forced the system into a constant cycle of loading and unloading models to serve alternating requests. This extreme VRAM churn frequently triggered timing deadlocks and process crashes for at least one of the users.

    \item \textbf{Scenario 3: Scalable Cluster with Triton Inference Server (HPP Deployment)}\\
          To fix the resource locking and local hardware constraints, we decoupled the multi-agent orchestration logic from the model inference engine. We moved the inference engine to the \textbf{inNuCE Lab Heterogeneous Prototyping Platform (HPP)} cluster. User-specific LangGraph containers now run exclusively on local CPU and RAM. They delegate all LLM requests over the network to a centralized \textbf{NVIDIA Triton Inference Server} via standardized APIs \cite{nvidia_triton_docs}. 
...
Table~\ref{tab:tool_comparison} highlights the architectural focus of different MLOps platforms. Commercial solutions like Edge Impulse provide robust auto-tuning (EON Tuner) and successfully generate compilable C++ deployment blocks for microcontrollers. However, they rely strictly on graphical web interfaces and leave the final IDE project integration to the developer. NVIDIA TAO delivers powerful transfer learning and tuning, yet it targets higher-end GPU architectures rather than constrained STM32 edge devices. Bare-metal libraries such as TFLite Micro provide the core runtime but offer zero deployment orchestration. STM32Cube.AI generates highly optimized C-code arrays but operates as a standalone CLI or GUI tool, requiring manual invocation sequence.
