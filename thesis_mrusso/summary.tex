...
The multi-agent paradigm solves these issues by decomposing the pipeline into a directed graph of specialized nodes, each responsible for one phase of the lifecycle. State is passed explicitly between nodes via a typed schema, enabling conditional routing, checkpointing, and supervised handoffs to a human operator at defined decision points. Applied to Edge AI development for STM32, this approach builds a system that translates a high-level natural language request (e.g., \textit{``Deploy a gesture classifier on STM32H7 with high compression''}) into a verified, compilable firmware project with minimal manual intervention.

\subsection*{Thesis Objectives and Contributions}

This thesis presents a multi-agent MLOps orchestration framework that automates the full Edge AI deployment cycle on STM32 microcontrollers. The system relies on \textbf{LangGraph} and manages seven specialized workflows as subgraphs connected through a shared typed state and a \textbf{Redis}-backed checkpointing layer. Human-in-the-Loop (HITL) gates, implemented via LangGraph's \texttt{interrupt()} primitive, allow engineers to approve decisions, redirect searches, or override model choices at defined breakpoints without losing execution context.
...
\begin{enumerate}
    ...
    \item A \textbf{dynamic web search evaluation} using DeepEval's LLM-as-Judge framework across five representative queries, measuring Faithfulness, Contextual Relevancy, Hallucination, and Answer Relevancy. Results show an average Contextual Relevancy of 0.75 and a Hallucination rate of 0.14, highlighting a measurable Faithfulness-Utility trade-off between procedural and exploratory query types.
