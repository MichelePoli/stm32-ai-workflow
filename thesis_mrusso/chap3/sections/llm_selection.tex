...
\begin{itemize}
    ...
    \item \textbf{Inference Latency vs. Reasoning Tokens}: Empirically, \textbf{GPT-OSS} showed lower wall-clock execution times compared to \textbf{DeepSeek-R1}. This is primarily due to the absence of the "thinking" (Chain-of-Thought) token overhead; while R1 generates internal reasoning tokens before outputting code, the 20B model generates the final script directly with high zero-shot accuracy.
    ...
    \item \textbf{Specialized Evaluation}: \textbf{DeepSeek-R1} is reserved for the \textit{LLM-as-a-Judge} role in my evaluation suite. Its ability to articulate reasoning is important for interpreting quality metrics (e.g., faithfulness, relevancy), where the "thinking" overhead is an acceptable trade-off for explainability.
\end{itemize}
