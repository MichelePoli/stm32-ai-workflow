...
\input{chap3/sections/workflow_graph_diagram.tex}
...
We categorize human interventions into two patterns. \textbf{Approval Gates} handle high-risk transitions, such as confirming hardware parameters or accepting a candidate model during iterative search. \textbf{Natural Language Steering} instead allows the user to redirect the agent mid-stream. For example, a user might state, \textit{``I actually need a smaller model''}, triggering a transition back to the search node with updated constraints.
