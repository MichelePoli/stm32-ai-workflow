\subsection{Firmware-AI Integration Workflow}
\label{sec:integration_flow}

Once the neural network code is ready (\textit{AI Model Discovery}, Section~\ref{sec:ai_flow}) and the firmware scaffolding is in place (\textit{Firmware Generation}, Section~\ref{sec:firmware_flow}), the \textit{Firmware-AI Integration} workflow merges the two outputs into a single deployable project, as illustrated in Figure~\ref{fig:detailed_flowchart_part3}. It handles path collection, firmware layout detection, AI file transfer, \texttt{main.c} patching, and a final consistency check, all without manual intervention.

The implementation resides in \texttt{workflow3\_integration.py} and runs five LangGraph nodes in sequence: \texttt{collect\_\allowbreak integration\_\allowbreak info}, \texttt{scan\_ai\_files}, \texttt{copy\_ai\_files}, \texttt{modify\_main\_c}, and \texttt{verify\_integration}.

\subsubsection{Project Path Collection and Idempotency}

The \texttt{collect\_\allowbreak integration\_\allowbreak info()} node determines two key paths: the firmware project directory and the folder containing the AI-generated code. When the workflow is triggered directly by the user (not as part of a sequential Firmware Generation $\rightarrow$ AI Model Discovery $\rightarrow$ Firmware-AI Integration chain), these paths are extracted from a natural language message using a Pydantic schema (\texttt{IntegrationInfoExtraction}) with structured LLM output.
...
\subsubsection{AI File Scanning}

The \texttt{scan\_\allowbreak ai\_\allowbreak files()} node lists the contents of the AI code directory and splits the files into two groups: \texttt{.c} sources (stored in \texttt{state.ai\_\allowbreak src\_\allowbreak files}) and \texttt{.h} headers (stored in \texttt{state.ai\_\allowbreak header\_\allowbreak files}). If both groups are empty, the workflow raises an error rather than producing a silent, broken integration.

\subsubsection{File Placement and X-CUBE-AI Runtime Headers}
\label{sec:copy_ai_files}

The \texttt{copy\_\allowbreak ai\_\allowbreak files()} node copies the scanned files into the firmware directories identified earlier: \texttt{.c} files go to \texttt{firmware\_\allowbreak src\_\allowbreak dir} and \texttt{.h} files go to \texttt{firmware\_\allowbreak inc\_\allowbreak dir}.

Beyond the network files, the node also handles a less obvious requirement: the X-CUBE-AI runtime headers. When STEdgeAI generates the network code, it also produces a subdirectory at \texttt{st\_\allowbreak ai\_\allowbreak ws/inspector\_\allowbreak network/workspace/include} containing the STMicroelectronics AI platform headers (such as \texttt{ai\_platform.h}). These files are needed at compile time, but are not included in a standard CubeMX project unless the X-CUBE-AI component has been explicitly added through the \texttt{.ioc} configuration.
...
\begin{enumerate}[noitemsep, topsep=4pt]
    \item \textbf{USER CODE BEGIN Includes}: adds \texttt{\#include "network.h"} and \texttt{"network\_\allowbreak data.h"}.
    ...
    \item \textbf{USER CODE BEGIN 2}: calls \texttt{ai\_\allowbreak network\_\allowbreak create()}, \texttt{ai\_\allowbreak network\_\allowbreak init()}, and binds the buffer pointers.
    \item \textbf{USER CODE BEGIN WHILE}: calls \texttt{ai\_\allowbreak network\_\allowbreak run()} in the main loop, with commented examples for sensor data input and output classification.
\end{enumerate}
...
\subsubsection{Verification and Persistent Context Update}
