...
Text is embedded using HuggingFace's \texttt{all-MiniLM-L6-v2} model and stored locally in \texttt{\textasciitilde/.cache/chroma\_db/architecture\_best\_practices}.

\subsubsection{Web Search Augmentation}

If the vectorstore lacks sufficient context for a specific architecture (e.g., uncommon CNNs like ShuffleNet), the system performs a time-limited web search (\texttt{\_fetch\_and\_cache\_with\_timeout()}) using Agno's GoogleSearchTools:

\begin{lstlisting}[language=Python, caption={Web-augmented RAG for best practices}, label={lst:rag_web}]
# From workflow5_customization.py
query = f"{model_name} fine-tuning best practices transfer learning"

def search_with_timeout():
    agent = Agent(
        model=Ollama(id="qwen3:latest"),
        tools=[GoogleSearchTools()],
        instructions="Find technical guides on fine-tuning this architecture"
    )
    return agent.run(query)

# Timeout after 10 seconds to avoid workflow stalls
results = execute_with_timeout(search_with_timeout, timeout=10)
\end{lstlisting}

Retrieved web pages are parsed with BeautifulSoup, chunked, and added to the vectorstore cache for future reuse.
