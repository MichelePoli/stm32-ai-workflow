...
The state persistence layer is implemented using **Redis**, a high-performance in-memory data store, chosen for its low latency and native support for asynchronous operations. The framework adopts a **Dual-Layer Memory Architecture**:
...
To bridge the gap between complex MLOps orchestration and the developer's daily workflow, a custom \textbf{Visual Studio Code Extension} was developed to act as the primary interface for the assistant. This integration transforms the framework from a disconnected backend service into a real-time, context-aware companion integrated directly into the Integrated Development Environment (IDE).
