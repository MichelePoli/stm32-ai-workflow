...
The framework is designed as a multi-layered, cloud-native architecture that bridges the gap between high-performance artificial intelligence models and resource-constrained embedded systems. As illustrated in Figure~\ref{fig:top_down_architecture}, the system is structured vertically across five operational layers, extending from the developer's local Integrated Development Environment (IDE) interface down to a centralized GPU computing cluster for heavy inference tasks.

At the core of this architecture is a stateful orchestration engine that coordinates four parallel agentic workflows. These workflows manage the entire development lifecycle:
...
\begin{enumerate}
    ...
    \item \textbf{AI Model Discovery and Optimization} (\texttt{ai\_flow}): Implements a two-tier model discovery system (curated task-based models plus iterative web search), followed by neural network customization and hyperparameter optimization with NNI.
...
\subsubsection{Reliability in LLM-Driven Orchestration}
...
\begin{itemize}
    ...
    \item \texttt{inject\_\allowbreak web\_\allowbreak search\_\allowbreak nodes()}: Web research and evaluation support (Workflow 4)
\end{itemize}
