...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \STATE \(blocks \gets \text{REGEX.findall}(\texttt{r'```[a-z]*\textbackslash n(.*?)```'}, R)\)
        \IF{\(|blocks| > 0\)}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \STATE \(pattern \gets \texttt{r'\^{}\# FILE:\textbackslash s*(\textbackslash S+\textbackslash .py)\textbackslash s*\$'} \)
        \STATE \(lines \gets C.\text{split}(\texttt{'\textbackslash n'})\)
