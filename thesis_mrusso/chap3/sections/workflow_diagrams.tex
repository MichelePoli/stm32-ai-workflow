...
\begin{tikzpicture}[
  ...
  co/.style={draw, rectangle, rounded corners=6pt, fill=purple!70, text=white,
             minimum width=2.2cm, minimum height=0.6cm, align=center, font=\sffamily\tiny\bfseries},
...
%% --- Web exit (left) ---
\node[co]   (WSOUT)  at (-6.2, -3.5)  {TO \\ Web Research};
...
\node[co]   (OUTAI)  at ( 0.0,-11.6)  {TO \\ Customize \\ Model?};
...
% ==============================================================
% FIGURE 2 — Model Customization, snake layout
% ==============================================================
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{%
...
\node[dec]  (CDS)  at (8.0, -7.5)  {Dataset \\ Source?};
\node[psub] (DREA) at (5.8, -9.3)  {WF7: Download Dataset \\ HuggingFace / Kaggle};
\node[psub] (DSYN) at (10.2,-9.3)  {WF6: Generate \\ Synthetic Data via LLM};
\node[dec]  (COPT) at (8.0,-11.1)  {Optimization \\ Method?};
\node[proc] (CNNI) at (5.8,-12.9)  {Run NNI Experiment \\ TPE Search, Multi-trial};
\node[proc] (CSTD) at (10.2,-12.9) {Fine-tune \\ Fixed Hyperparameters};
\node[proc] (CVAL) at (8.0,-14.7)  {Validate Custom Model \\ Accuracy / Loss};
\node[proc] (CSAV) at (8.0,-16.2)  {Save Final Model \\ .h5 + metadata};
\node[dec]  (CCON) at (8.0,-17.9)  {Continue to \\ STEdgeAI Analysis?};
\node[co]   (OUY)  at (5.8,-19.5)  {TO \\ Run STEdgeAI};
\node[se]   (ENDB) at (10.2,-19.5) {END};
...
%% Snake turn
\draw[ar] (CUC.east) -- (4.0, -8.5) |- (CDS.west);
...
\begin{scope}[on background layer]
  \node[wb, fit=(CUA)(CINT)(CUB)(CUC)(CDS)(DREA)(DSYN)(COPT)(CNNI)(CSTD)(CVAL)(CSAV)(CCON)(OUY)(ENDB),
        label={[font=\sffamily\tiny\bfseries,text=gray!60, yshift=-0.5mm,xshift=+40mm]above:Model Customization Workflow}]{};
\end{scope}
\end{tikzpicture}}
\caption{Detailed Workflow Diagram - \textit{Model Customization} workflow. Left column: architecture inspection and modification. Right column: dataset acquisition, hyperparameter optimization via NNI, and model validation.}
...
%% --- FROM connectors ---
\node[co]   (F3A)  at (-2.9,  0)   {FROM \\ Customize \\ Model};
\node[co]   (F3B)  at (-0.1,  0)   {FROM \\ Customize \\ Skip};
...
\node[co]   (F3C)  at ( 7.5, -1.0) {FROM \\ Route User \\ Request};
...
\end{scope}
\end{tikzpicture}}
\caption{Detailed Workflow Diagram - \textit{AI Model Discovery} (STEdgeAI profiling and C-code generation), \textit{Firmware-AI Integration}, and on-demand \textit{Web Research}.}
