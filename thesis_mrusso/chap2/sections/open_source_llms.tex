...
\textbf{Ollama} is a lightweight, open-source framework designed to simplify the local deployment and management of LLMs. It packages model weights, configurations, and inference engines into a single, easy-to-use interface. By utilizing the \texttt{llama.cpp} backend, Ollama provides high-performance inference through hardware acceleration (Metal on macOS, CUDA on NVIDIA, and ROCm on AMD). 
...
\end{enumerate}
In the developed framework, Ollama serves as the primary inference backend, ensuring that all model "surgery" and firmware configuration logic remains within the user's local network, thereby preserving intellectual property and reducing external dependencies.
